...
public class MembermanagementController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("birthOrOther")
    public String birthOrOther(Model model) {
        BaseEntityWrapper<User> deptBaseEntityWrapper = new BaseEntityWrapper<>();
        List list = userService.selectList(deptBaseEntityWrapper);
        model.addAttribute("staffs", list);
        EntityWrapper<Dept> deptBaseEntityWrapper1 = new EntityWrapper<>();
        ...
        model.addAttribute("depts", depts);
        //查询病史
        List<BaMedical> baMedicals = baMedicalService.selectList(null);
        model.addAttribute("baMedicals", baMedicals);
        return PREFIX + "membermanagement1.html";
    }
    ...
    public Object list(String name,
                       ...
                       String district,
                       String memberid,
                       String townshipid, String year, String month, String day, String medicalHistory) {
        ...
        if (!StringUtils.isEmpty(memberBirth)) baseEntityWrapper.like("birthday", memberBirth);
        if (!StringUtils.isEmpty(medicalHistory)) baseEntityWrapper.like("medicalHistory", medicalHistory);
    ...
    public Object add(Membermanagement membermanagement, String cardCode, String baMedicals, String code
        ...
        membermanagement.setMedicalHistory(baMedicals);
...
//        memberCardBaseEntityWrapper.eq("code", cardCode);
        ...
        if (!StringUtils.isEmpty(baMedicals)) {
            ...
            for (String s : split) {
                ...
                memberBamedical.setDeptId(ShiroKit.getUser().getDeptId());
...
//                membermanagement.setCountPrice(excelParams.getmCountPrice());
    ...
    //病况查询
    @RequestMapping("/membermanagement1_medicalHistory")
    public String membermanagement1_medicalHistory() {

        return PREFIX + "membermanagement1_medicalHistory.html";
    }

    @RequestMapping(value = "/medicalHistoryPage")
    @ResponseBody
    public Object medicalHistoryPage(String condition) {
        Page<BaMedical> page = new PageFactory<BaMedical>().defaultPage();
        EntityWrapper<BaMedical> baseEntityWrapper = new EntityWrapper<>();
        Page<BaMedical> result = baMedicalService.selectPage(page, baseEntityWrapper);
        List<BaMedical> records = result.getRecords();
        records.forEach(a -> {
            a.getId();
            BaseEntityWrapper<MemberBamedical> entityWrapper = new BaseEntityWrapper<>();
            entityWrapper.eq("bamedicalid",a.getId());
           a.setIsactive(memberBamedicalService.selectCount(entityWrapper));
        });
